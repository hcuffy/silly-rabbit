...
describe("runSessionReplay — REPLAY_MODE='mocked' (session-replay-spec §4.3/§5.3), real chromium + " +
  ...
  it("REPLAY_MODE default ('live', unspecified): a step's fetch call genuinely reaches the live route handler " +
    "— proves the baseline live path still hits real network when mocked mode isn't requested", async () => {
  ...
  it("REPLAY_MODE='mocked': the same step's fetch call is fulfilled entirely from the recorded response, " +
    "zero live hits — and the mocked-mode installer is genuinely additive, not intertwined: the exact same " +
    "recording/steps/deps used in the live-mode test above behave differently only because of the mode flag", async () => {
    const runId = randomUUID();
    let liveApiHits = 0;
    await page.route(`${ALLOWED_URL}api/data`, (route) => {
      liveApiHits++;
    ...
    const networkCaptures: NetworkCapture[] = [
      { url: `${ALLOWED_URL}api/data`, method: "GET", status: 200, bodyPath, timestampOffsetMs: 500 },
    ];
  ...
  it("REPLAY_MODE='mocked': navigation and destructive-action guards still apply unchanged — a destructive " +
    "click is still refused even though nothing real would be written in mocked mode", async () => {
    const runId = randomUUID();
    const recording = makeRecording({
      steps: [
        { action: "navigate", selectorStrategy: "css", value: ALLOWED_URL, timestampOffsetMs: 0 },
