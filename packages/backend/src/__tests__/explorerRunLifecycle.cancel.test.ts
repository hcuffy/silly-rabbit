...
describe("explorerRunLifecycle — cancelExplorerRun (delete-cancel-spec.md §4, phase 1)", () => {
  ...
  it(
    "cancelling while stuck on the initial navigation actually closes the real chromium instance — " +
      "the Run reaches CANCELLED rather than hanging forever, and no TestRun is ever created (cancelled " +
      "before locateSection could get that far)",
    async () => {
  ...
  it(
    "cancels both the Run and its TestRun together — proven against a directly-seeded RUNNING pair, " +
      "no live browser involved, the same degrade-gracefully path a stale post-crash RUNNING record " +
      "would go through (no registry entry, DB-level cancel is still the source of truth)",
    async () => {
      const run: Run = {
        id: randomUUID(),
