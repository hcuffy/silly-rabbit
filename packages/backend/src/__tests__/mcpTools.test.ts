...
describe(
  "MCP tools (mcp-server-spec §4.4) — read/list/feedback tools and error paths, no browser " +
  ...
  () => {
    ...
    it(
      "lists all 14 tools by name (9 from the spec's locked table, the LIST-ALL audit-fix additions " +
        "list_explorer_runs/list_session_replay_runs, and app-mapping-spec.md §8/§11's crawl_nav_map/get_nav_map/delete_nav_map)",
      async () => {
    ...
    it(
      "submit_finding_feedback confirmed_issue on a finding with no featureId returns isError, mirroring " + "the HTTP route's exact same 400 case",
      async () => {
        const finding = {
          id: randomUUID(),
          runId: "run-1",
    ...
    it(
      "trigger_session_replay_run on an unknown sessionId returns isError, not a thrown protocol error " +
        "(business-logic 'not found', distinct from a schema-validation failure)",
      async () => {
        const result = await client.callTool({
    ...
    it(
      "invalid input (non-uuid sessionId) is rejected by the tool's own inputSchema before the handler " +
        "ever runs — confirmed empirically (not assumed): the SDK returns this as isError:true content, " +
        "not a thrown/rejected protocol-level error",
      async () => {
        const result = await client.callTool({
          name: "trigger_session_replay_run",
