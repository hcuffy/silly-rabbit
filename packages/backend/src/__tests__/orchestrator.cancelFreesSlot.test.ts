...
describe(
  "cancelling a RUNNING run frees its reserveRunSlot() concurrency slot — real interaction " +
  ...
  () => {
    ...
    it(
      "cap=1: a 2nd trigger is rejected while the 1st is stuck RUNNING; cancelling the 1st frees the " + "slot so a 3rd trigger now succeeds",
      async () => {
        ...
        await expect(startRun({ charter: "test the locations flow", targetBaseUrl: MOCK_BASE_URL }, deps)).rejects.toBeInstanceOf(RunCapacityError);
