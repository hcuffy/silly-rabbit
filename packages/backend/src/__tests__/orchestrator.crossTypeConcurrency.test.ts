...
describe("run-type concurrency cap is genuinely shared — a charter run (D1-D7) and a session-replay " +
  ...
  it("rejects a session-replay trigger fired while a charter run already holds the single shared slot, " +
    "then accepts a third trigger once the first one finishes", async () => {
