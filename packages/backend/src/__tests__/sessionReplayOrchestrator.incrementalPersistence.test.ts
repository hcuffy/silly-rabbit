...
describe("runSessionReplay — SessionReplayRun incremental persistence (session-replay-spec §8.3 " +
  ...
  it("SessionReplayRun status/summary are visible mid-run, before completion — same proof pattern as " +
    "D8's incremental-persistence fix (explorerOrchestrator.resilience.test.ts): don't await the whole " +
    "run first, poll the repo concurrently", async () => {
