...
describe("cascadeDelete — mid-crash safety (delete-cancel-spec.md §8, non-atomic by real infra constraint)", () => {
  ...
  it("deleteRunCascade: a crash right before the final Run delete leaves Findings+TestRun already " +
    "gone but the Run still present (safe-partial, not corrupted) — a retry then finishes the job", async () => {
    const run = makeRun();
    ...
    await expect(
      deleteRunCascade(run.id, { runRepo: crashingRunRepo, testRunRepo, findingRepo }),
  ...
  it("deleteSessionReplayRunCascade: a crash right before the final run delete leaves its Findings " +
    "already gone but the run still present; a retry finishes the job", async () => {
    ...
    await expect(
      deleteSessionReplayRunCascade(run.id, { sessionReplayRunRepo: crashingRepo, findingRepo }),
    ).rejects.toThrow("simulated crash");
  ...
  it("deleteSessionRecordingCascade: a crash right before the final recording delete leaves its " +
