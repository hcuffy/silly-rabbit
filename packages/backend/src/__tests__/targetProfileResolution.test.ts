...
import { resolveActiveProfileForRequest, resolveActiveProfileOverrides, withActiveProfileOverrides } from "../targetProfileResolution.js";
...
describe("resolveActiveProfileOverrides / withActiveProfileOverrides (target-profiles-spec.md §4, backward-compat step)", () => {
  ...
  it(
    "withActiveProfileOverrides is a true no-op when no active profile exists — env-sourced deps " +
      "fields (loginCreds, allowedDomains, productionUrlPatterns) all pass through completely untouched",
    async () => {
      const bootDeps = {
        activeTargetProfileRepo,
        targetProfileRepo,
        loginCreds: {
          loginUrl: "https://env.example.com/login",
  ...
  it(
    "withActiveProfileOverrides no-ops when the repos aren't present on deps at all — the shape every " +
      "pre-existing test's hand-built deps object has, proving those tests need no changes",
    async () => {
      const bootDeps: {
  ...
  it(
    "an active, fully-configured profile overrides loginCreds/allowedDomains/charterNav.locationsPath — " +
      "PROD_URL_PATTERNS-equivalent (productionUrlPatterns) is never touched, confirmed by its absence here",
    async () => {
  ...
  it(
    "an active profile with incomplete login config (no password) explicitly overrides loginCreds to " +
      "undefined — replacing env's login entirely, not merging with it",
    async () => {
...
describe(
  "resolveActiveProfileForRequest — closes the double-round-trip active-profile race " +
  ...
  () => {
    it(
      "fetches the active profile exactly once — targetBaseUrl and the rest of the overrides always come " +
        "from the same read, even when a second read would return a different profile",
      async () => {
        ...
        const activePointerGet = vi.fn(() => Promise.resolve(ActiveTargetProfileSchema.parse({ profileId: profileA.id, updatedAt: new Date() })));
