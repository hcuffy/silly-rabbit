...
import { assertAllowedUrl, assertNotDestructive, assertNotProductionUrl, assertRollbackDeleteAllowed, SafetyViolation } from "../safety.js";
...
describe(
  "safety-guard boundary — the D8 rollback exception is scoped, everything else is not " + "(explorer-spec §9/§13.9, real safety.ts + real chromium)",
  () => {
    let browser: Browser;
    let page: Page;
    ...
    it(
      "rollback()'s own delete click, wired to the real assertRollbackDeleteAllowed, " + "succeeds because the caller already verified the match",
      async () => {
    ...
    it(
      "THE test that matters: a boundary check's own submit click is still hard-refused by ordinary assertNotDestructive " +
        "when it happens to hit something Delete-shaped — general execution has NO exception, only rollback's own click does",
      async () => {
        await page.setContent(`
      <html><body>
        <h1>Locations</h1>
