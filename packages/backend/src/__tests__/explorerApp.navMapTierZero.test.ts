...
describe("NavMap tier-0 wiring end-to-end (app-mapping-spec.md §7) — a real crawled NavMap in Mongo lets a " +
  ...
  it("a real NavMap entry for the target lets locateSection resolve via tier-0, so the run completes without " +
    "ever calling the section-match LLM (which would throw and fail the run if it were reached)", async () => {
