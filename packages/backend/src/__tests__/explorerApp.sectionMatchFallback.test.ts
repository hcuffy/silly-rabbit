  ...
  return {
    messages: {
      create: (parameters) => {
        ...
        if (toolName === "submit_section_match") {
          return Promise.resolve(sectionMatchResponse());
        }
...
describe(
  "Section-match LLM fallback wiring (explorer-spec §12.1) — a triggered fallback call is counted, " + "not lost before the tracked client existed",
  () => {
    ...
    it(
      "a word-level miss ('the vehicles list and detail view' vs the real label 'Fahrzeuge') escalates to the " +
        "LLM fallback, resolves the section, and the fallback call's usage lands in the run's llmCallsUsed/costUsd " +
        "— proving trackClientUsage now wraps the client before locateSection runs, not after",
      async () => {
