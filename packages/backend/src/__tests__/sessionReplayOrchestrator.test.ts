...
describe("runSessionReplay (session-replay-spec — replay execution, REPLAY_MODE=live), real chromium + " +
  ...
  beforeEach(async () => {
    ...
    await page.route(`${BLOCKED_URL}**`, (route) =>
      route.fulfill({ contentType: "text/html", body: "<html><body>Blocked Landed</body></html>" }),
    );
  });
  ...
  it("a destructive-guard refusal mid-replay degrades to a Finding and the replay continues — later steps " +
    "still execute, every step's result persists incrementally (D8 incremental-persistence precedent)", async () => {
    const runId = randomUUID();
    const recording = makeRecording({
      steps: [
        { action: "navigate", selectorStrategy: "css", value: ALLOWED_URL, timestampOffsetMs: 0 },
  ...
  it("a navigation-guard refusal (disallowed host) also degrades cleanly rather than crashing the replay " +
    "— confirms the navigation guard call site is real, not assumed", async () => {
    const runId = randomUUID();
    const recording = makeRecording({
      steps: [
        { action: "navigate", selectorStrategy: "css", value: ALLOWED_URL, timestampOffsetMs: 0 },
    ...
    const persisted = await findingRepo.findByRunIds([runId]);
    expect(persisted.some((finding) => finding.reasoning?.includes("ALLOWLIST") || finding.reasoning?.includes("allowlist"))).toBe(
      true,
    );
  });
  ...
  it("a real divergence between two replay runs of the same recording triggers the judge path, persists a " +
    "STATE_DIVERGENCE Finding with origin 'session-replay'", async () => {
    const recording = makeRecording({
    ...
    const summary = await runSessionReplay(
      ...
      deps({ judgeClientFactory: () => client }),
    );
  ...
  it("a thrown error from captureAndCompare's Mongo-touching callback (getBaseline) degrades to a " +
    ...
    const getByScreenIdsSpy = vi.spyOn(baselineRepo, "getByScreenIds").mockImplementation((...arguments_) => {
      callCount++;
      if (callCount === 2) throw new Error("simulated transient Mongo error");
