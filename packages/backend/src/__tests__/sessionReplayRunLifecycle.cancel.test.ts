...
describe("sessionReplayRunLifecycle — cancelSessionReplayRun (delete-cancel-spec.md §4, phase 1)", () => {
  ...
  it("FIXED (was a documented known gap): cancelling mid-run now sticks — runSessionReplay checks real " +
    ...
    "CANCELLED status cancelSessionReplayRun() already wrote", async () => {
    ...
    if (!run) throw new Error("unreachable — sessionRecording was just created");
  ...
  it("the early-exit between-steps check actually kicks in — a 3-step recording where step 1 hangs " +
    ...
    "early-exit, not just an artifact of there being only one step to begin with)", async () => {
    const sessionRecording: SessionRecording = {
      sessionId: randomUUID(),
      targetBaseUrl: MOCK_BASE_URL,
      recordedAt: new Date(),
