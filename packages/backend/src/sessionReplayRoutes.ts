...
export function registerSessionReplayRoutes(app: FastifyInstance, deps: AppDeps): void {
  app.post("/session-replay/runs", async (request, reply) => {
    ...
    try {
      ...
      if (!run) return reply.status(404).send({ error: "session recording not found" });
  ...
  app.get<{ Querystring: { limit?: string; offset?: string; cycleId?: string } }>(
    "/session-replay/runs",
    async (request, reply) => {
  ...
  app.get<{ Params: { id: string } }>("/session-replay/runs/:id", async (request, reply) => {
    ...
    if (!run) return reply.status(404).send({ error: "session-replay run not found" });

    const [findings, sessionRecording] = await Promise.all([
      deps.findingRepo.findByRunIds([run.id]),
      deps.sessionRecordingRepo.get(run.sessionId),
    ]);
  ...
  app.post<{ Params: { id: string } }>("/session-replay/runs/:id/cancel", async (request, reply) => {
    if (await cancelSessionReplayRun(request.params.id, deps)) return reply.status(200).send({ cancelled: true });
  ...
  app.delete<{ Params: { id: string } }>("/session-recordings/:id", async (request, reply) => {
    ...
    if (!recording) return reply.status(404).send({ error: "session recording not found" });
