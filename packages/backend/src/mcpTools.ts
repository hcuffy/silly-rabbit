...
export function registerMcpTools(server: McpServer, deps: McpToolDeps): void {
  ...
  server.registerTool(
    ...
    async ({ limit, offset }) =>
      jsonResult(await deps.testRunRepo.list({ limit: limit ?? DEFAULT_LIST_LIMIT, offset: offset ?? 0 })),
  );
  ...
  server.registerTool(
    ...
    async (arguments_) => {
      ...
      const effectiveDeps = resolution.overrides
        ? { ...deps, allowedDomains: resolution.overrides.allowedDomains }
        : deps;
      ...
      try {
        ...
        if (!run) return errorResult("session recording not found");
        return jsonResult({ runId: run.id, status: run.status });
      } catch (error) {
        if (error instanceof RunCapacityError) return errorResult(error.message);
        throw error;
  ...
  server.registerTool(
    ...
    async ({ limit, offset }) =>
      jsonResult(await deps.sessionReplayRunRepo.list({ limit: limit ?? DEFAULT_LIST_LIMIT, offset: offset ?? 0 })),
  );
