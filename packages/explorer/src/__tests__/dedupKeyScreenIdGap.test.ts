...
describe(
  "known gap (explorer-spec §10.3/§10.4, deferred): computeCheckDedupKey's screenId comes from " +
  ...
  () => {
    ...
    it(
      "a failing check whose submit navigates produces a real Finding.dedupKey that computeCheckDedupKey " +
        "(computed purely from the pre-run research) cannot reproduce",
      async () => {
