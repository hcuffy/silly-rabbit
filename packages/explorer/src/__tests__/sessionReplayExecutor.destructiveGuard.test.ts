...
describe("executeSessionReplayStep — css-strategy click fail-closed fix (audit finding #1), real chromium. " +
  ...
  it("a css-strategy click step is refused (fail-closed) rather than clicking blind — no accessible " +
    ...
    "click whose recorded selector fell back to css)", async () => {
    ...
    const result = await executeSessionReplayStep(
      baseInput(page, { action: "click", selectorStrategy: "css", cssSelector: "#delete-row", timestampOffsetMs: 0 }, {
        onBeforeAction,
      }),
    );
  ...
  it("a role-strategy click is unaffected by the css-strategy fail-closed fix above — still routes through " +
    "onBeforeAction and clicks normally", async () => {
    page = await browser.newPage();
