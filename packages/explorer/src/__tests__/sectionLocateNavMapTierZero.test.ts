...
describe("locateSection tier-0 (app-mapping-spec.md §7) — real chromium", () => {
  ...
  it(
    "a map hit that verifies live resolves via matchSource 'map', skipping ariaSnapshot and the LLM entirely " +
      "— this is the real cost saving over tier-1/2's full-tree-scan-then-LLM path",
    async () => {
      page = await browser.newPage();
      await page.route("https://allowed.example/**", (route) => route.fulfill({ contentType: "text/html", body: "OK" }));
      await page.setContent(`<html><body><ul><li onclick="window.location.href='https://allowed.example/section'">Standorte</li></ul></body></html>`);
  ...
  it(
    "map hit but the entry no longer resolves live falls through to tier-1's real ariaSnapshot scan, " +
      "self-healing rather than trusting or crashing on the stale answer",
    async () => {
      page = await browser.newPage();
  ...
  it(
    "a map that exists but has no entry matching the description at all falls through cleanly to a real " +
  ...
  it("no navMap option at all takes the exact same path as before phase 2 — the byte-identical-when-absent " + "case", async () => {
    page = await browser.newPage();
    await page.setContent(`<html><body><ul><li>Nutzer</li></ul></body></html>`);
