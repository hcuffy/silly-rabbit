...
describe("sweepNavMapEntries (app-mapping-spec.md §6.2/§6.4) — real chromium", () => {
  ...
  it(
    "structure drift: a real fingerprint mismatch is detected — entry self-heals with the fresh structure, " +
      "isStale false, lastVerifiedAt refreshed",
    async () => {
      page = await browser.newPage();
  ...
  describe(
    "locale-relabel corroboration (navmap-locale-spec.md §4, same pattern as sectionLocate.ts's verify-miss " +
      "path) — a literal-label mismatch is no longer immediately treated as drift",
    () => {
      it(
        "a locale switch (live label no longer matches, but the entry's own normalizedUrl still resolves to the " +
          "same real, fingerprint-matching destination) self-heals — not marked stale",
        async () => {
          page = await browser.newPage();
          const routes: Record<string, { contentType: string; body: string }> = {
      ...
      it(
        "a genuinely different destination (the stored URL now resolves to different, non-matching content — a " +
          "real removal/regression, not a relabel) still marks the entry stale, not relabeled",
        async () => {
          page = await browser.newPage();
          await page.route(`${ORIGIN}/**`, (route) => {
            const path = new URL(route.request().url()).pathname;
            if (path === "/") {
              return route.fulfill(html(`<a href="/somewhere">Different Live Label</a>`));
            }
      ...
      it(
        "an entry with no stored normalizedUrl at all cannot be corroborated (the sweep only ever revisits a " +
          "known destination directly — unlike sectionLocate.ts it has no click-through re-resolution step to " +
          "observe a live label from) and is correctly still marked stale, not relabeled",
        async () => {
          page = await browser.newPage();
