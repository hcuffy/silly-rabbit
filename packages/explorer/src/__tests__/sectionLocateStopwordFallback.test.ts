...
describe("locateSection — stopword-fallback tiering (real bug found via live dogfooding: generic-noun " + "collision on a compound label)", () => {
  let browser: Browser;
  let page: Page;
  ...
  it(
    "a description whose only significant words are all anti-collision generic nouns ('list', 'details', " +
      ...
      "instead, which correctly resolves to the real 'Standorte' label",
    async () => {
      page = await browser.newPage();
      await page.setContent("<html><body><ul><li>Standorte</li><li>Unternehmens-details</li></ul></body></html>");
  ...
  it(
    "a description made entirely of connector words ('with and for') empties out at both stopword tiers — " +
