...
describe("executeSessionReplayStep — action routing & dedup (session-replay-spec — replay execution, live mode), real chromium", () => {
  let browser: Browser;
  let page: Page;
  ...
  it(
    "an execution error (e.g. onBeforeAction throwing, matching a real safety-guard refusal) degrades to " +
      "the same failure-Finding shape rather than propagating and killing the caller's loop",
    async () => {
      page = await browser.newPage();
      ...
      const result = await executeSessionReplayStep(
        baseInput(
          page,
          { action: "click", selectorStrategy: "role", role: "button", accessibleName: "Delete row", timestampOffsetMs: 0 },
          {
            onBeforeAction,
          },
        ),
  ...
  it(
    "routes a role-strategy click through onBeforeAction with the recorded role/accessibleName (same " + "destructive-guard call site D8 uses)",
    async () => {
      page = await browser.newPage();
  ...
  it("routes a navigate step through onBeforeNavigate with the recorded URL", async () => {
    ...
    await executeSessionReplayStep(
      baseInput(
        page,
        { action: "navigate", selectorStrategy: "css", value: "https://replay.local/", timestampOffsetMs: 0 },
        {
          onBeforeNavigate,
        },
      ),
  ...
  it("an already-open Finding with the same dedupKey is marked RECURRING, not duplicated, origin stays " + "'session-replay'", async () => {
    page = await browser.newPage();
    ...
    const second = await executeSessionReplayStep(
      baseInput(
        page,
        { action: "click", selectorStrategy: "role", role: "button", accessibleName: "Refresh", timestampOffsetMs: 100 },
        {
          judge: { clientFactory: throwingJudgeClient },
          getBaseline: () => Promise.resolve(baseline),
