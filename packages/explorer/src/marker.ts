...
export function selectFreeTextField(inputValues: Record<string, string>, research: ResearchInventory): string | undefined {
  for (const fieldName of Object.keys(inputValues)) {
    if (EXCLUDED_FIELD_NAME_PATTERN.test(fieldName)) {
      continue;
    }
...
export function injectMarker(inputValues: Record<string, string>, fieldName: string, marker: string): Record<string, string> {
