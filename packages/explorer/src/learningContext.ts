...
export async function refreshLearningConfirmations(
  ...
  const learningsByDedupKey = new Map(
    activeLearnings
      .filter((learning): learning is Learning & { dedupKey: string } => Boolean(learning.dedupKey))
      .map((learning) => [learning.dedupKey, learning]),
  );
  if (learningsByDedupKey.size === 0) {
    return;
  }
  ...
  for (const outcome of testRun.checkOutcomes) {
    ...
    if (!learning || learning.lastConfirmedRunId === testRun.runId) {
      continue;
    }
