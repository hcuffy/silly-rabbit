...
function stubFetchWithRunDetail(overrides: Partial<Record<string, unknown>> = {}) {
  vi.stubGlobal(
    "fetch",
    vi.fn(() => Promise.resolve(jsonResponse(makeRunDetail(overrides)))),
  );
}

describe(
  "SessionReplayDetail (dashboard-integration slice 2, session-replay-spec §8.2/§8.3 " +
  ...
  () => {
    ...
    it(
      "a step with an associated Finding: the Finding renders via the existing FindingCard badge " +
        "treatment (CONFIRM-9 — no new status-indicator component built)",
      async () => {
        stubFetchWithRunDetail({
    ...
    it(
      "an uncycled run keeps today's exact bare header — no new fallback invented (run-cycles-spec.md " +
        "§9: D9 never had a flat scheme, so there is nothing to fall back to)",
      async () => {
        stubFetchWithRunDetail();
        renderWithClient(<SessionReplayDetail runId={RUN_ID} />);
