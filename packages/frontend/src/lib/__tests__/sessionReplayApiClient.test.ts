...
describe(
  "session-replay apiClient functions (dashboard-integration slice 2, session-replay-spec §8). " +
  ...
  () => {
    ...
    it("triggerSessionReplayRun throws ApiError with the backend's error message on a 404 " + "(sessionId with no matching recording)", async () => {
      const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>;
      fetchMock.mockResolvedValueOnce(jsonResponse({ error: "session recording not found" }, 404));

      await expect(triggerSessionReplayRun({ sessionId: "9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d" })).rejects.toBeInstanceOf(ApiError);
    });
