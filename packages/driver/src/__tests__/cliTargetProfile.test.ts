...
describe("cli main() --profile flag (target-profiles-spec.md phase 3, item 2)", () => {
  ...
  it(
    "--profile resolves by name and its baseUrl takes effect — a real run genuinely completes " +
      "against that distinct, non-default baseUrl (mock-target route interception is broad enough " +
      "to service any hostname, so this proves application, not just a write-then-crash)",
    async () => {
  ...
  it("an unresolvable --profile name/id throws a clear error before touching Mongo run-storage or " + "a browser", async () => {
  ...
  it(
    "the profile's OWN allowedDomains governs the safety check, not env's ALLOWED_DOMAINS — proven " +
      "by making env's list permissive and the profile's list restrictive for the same host",
    async () => {
