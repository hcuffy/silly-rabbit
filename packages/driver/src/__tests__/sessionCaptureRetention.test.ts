...
describe(
  "enforceSessionCaptureStorageCap (mirrors enforceScreenshotStorageCap's Option C + " + "purge-on-write, applied across session subdirectories)",
  () => {
    ...
    it(
      "deletes the oldest capture(s) first once the cap is exceeded, keeping the newest, across session " + "subdirectories (not just within one)",
      async () => {
        const directory = await makeTemporaryDirectory();
