...
async function waitForStepCount(capture: SessionCaptureHandle, count: number) {
  return vi.waitFor(() => {
    ...
    if (steps.length < count) throw new Error(`expected at least ${count} step(s), have ${steps.length}`);
    return steps;
  });
}

describe("attachSessionCapture (session-replay-spec §4.1/§5.2, real chromium) — clicks/fills/navigations " +
  ...
  it("coalesces per-keystroke input events on the same field into one step with the final value " +
    "(real human typing fires one native `input` event per keystroke — this must not produce N steps)", async () => {
    page = await browser.newPage();
    const capture = await attachSessionCapture(page, new Date(), new URL(FIXTURE_URL).origin);
    await gotoFixture(page, FIXTURE_HTML);
  ...
  it("never captures a real password value — redacted in-browser at the point of capture, not just " +
    "different from what was typed (audit finding #2: a real credential typed during recording was " +
    "previously persisted verbatim)", async () => {
    page = await browser.newPage();
    const capture = await attachSessionCapture(page, new Date(), new URL(FIXTURE_URL).origin);
  ...
  it("re-attaches its listeners across a full navigation (addInitScript re-injects on every new document, " +
    "not just the first one) — the navigation between the two fixtures is itself a real, correctly " +
    "recorded step (only the recorder's own very first landing navigation is skipped, §4.1)", async () => {
    page = await browser.newPage();
    const capture = await attachSessionCapture(page, new Date(), new URL(FIXTURE_URL).origin);
...
describe("attachSessionCapture — network capture (session-replay-spec §5.3, resolved), real chromium", () => {
  ...
  async function waitForCaptureCount(capture: SessionCaptureHandle, count: number) {
    return vi.waitFor(() => {
      ...
      if (captures.length < count) throw new Error(`expected at least ${count} capture(s), have ${captures.length}`);
      return captures;
    });
  }

  it("captures a same-origin fetch JSON response, but not a same-origin image fetch or a cross-origin " +
  ...
  it("does not capture a non-JSON same-origin fetch response even when resourceType is fetch/xhr " +
    ...
    "not two, rather than a fixed sleep to assert a negative", async () => {
    page = await browser.newPage();
    await page.route(`${FIXTURE_URL}**`, (route) => {
      const url = route.request().url();
