...
describe("installMockedReplayRoute (session-replay-spec §4.3/§5.3), real chromium", () => {
  ...
  it("fulfills a matching xhr/fetch request from the recorded response, never reaching the live handler", async () => {
    ...
    const networkCaptures: NetworkCapture[] = [
      { url: `${FIXTURE_URL}api/data`, method: "GET", status: 200, bodyPath, timestampOffsetMs: 0 },
    ];
  ...
  it("a request with the same method+url matched twice consumes the queue in order (FIFO), not the same " +
    "response repeated — e.g. a recorded poll returning two different bodies in sequence", async () => {
    page = await browser.newPage();
    ...
    const results = await page.evaluate(async (): Promise<{ count: number }[]> => [
  ...
  it("a 3rd request against a 2-entry queue for the same method+url fails closed, same as an entirely " +
    "unmatched request — explicit, not just inferred from the unmatched-request case being the same code " +
    "path (audit coverage gap: queue exhaustion was previously untested)", async () => {
    page = await browser.newPage();
    await page.route(`${FIXTURE_URL}**`, (route) => route.fulfill({ contentType: "text/html", body: "<html><body>ok</body></html>" }));
  ...
  it("still lets document requests hit the live handler unmodified — mocked mode only covers same-origin " +
    "xhr/fetch, not the page shell itself", async () => {
    page = await browser.newPage();
    let documentHits = 0;
    await page.route(`${FIXTURE_URL}**`, (route) => {
      documentHits++;
  ...
  it("navigation requests are not swallowed by the mocked route — a second goto still reaches the live " +
    "handler, proving mocked mode defers navigation via fallback rather than intercepting it itself", async () => {
    page = await browser.newPage();
    let hits = 0;
    await page.route(`${FIXTURE_URL}**`, (route) => {
      hits++;
